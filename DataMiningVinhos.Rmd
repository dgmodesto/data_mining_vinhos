...
cor(train_normalizado$quality,train_normalizado$totalsulfurdioxide)
cor(train_normalizado$quality,train_normalizado$freesulfurdioxide)
cor(train_normalizado$quality,train_normalizado$residualsugar)
cor(train_normalizado$quality,train_normalizado$citricacid)
cor(train_normalizado$quality,train_normalizado$alcohol)
cor(train_normalizado$quality,train_normalizado$id_vinho)
cor(train_normalizado$quality,train_normalizado$pH)
cor(train_normalizado$quality,train_normalizado$density)
cor(train_normalizado$quality,train_normalizado$sulphates)
cor(train_normalizado$quality,train_normalizado$fixedacidity)
cor(train_normalizado$quality,train_normalizado$chlorides)
cor(train_normalizado$quality,train_normalizado$volatileacidity)
cor(train_normalizado$quality,train_normalizado$Vinho)
```

As variáveis com maior correlação ao nosso target é "Alchool" e "densidade". A primeira é a com maior correlação positiva e a segunda é a com maior correlação negativa.

```{r}
#matriz de correlações

matcor <- cor(train_normalizado)
...
smp_size <- floor(0.7 * nrow(train_normalizado))

## coloca o seed para fazer sua partição reproduzível
set.seed(1)
train_ind <- sample(seq_len(nrow(train_normalizado)), size = smp_size)

train <- train_normalizado[train_ind, ]
test <- train_normalizado[-train_ind, ]
```
...
Conforme mencionado anterioremente a função remove a propriedade com menor valor de correlação parcial, a função backward removeu a propriedade id_vinho que apresentou o valor de  0.000000006, por ela ter o menor valor de correlação parcial dentro do modelo.
...
AIQ_residualsugar<-quantile(residual_sugar$alcohol,.75,type=2)-quantile(residual_sugar$alcohol,.25,type=2)
AIQ_residualsugar

limsup_residualsugar= quantile(residual_sugar$alcohol,.75,type=4)+1.5*AIQ_residualsugar
limsup_residualsugar
liminf_residualsugar= quantile(residual_sugar$alcohol,.25,type=2)-1.5*AIQ_residualsugar
liminf_residualsugar

residual_sugar_1<-subset(residual_sugar, alcohol>5)
...
______________________________________________________________________________________________________
```{r}
fx_alcohol2 <- cut(residual_sugar_1$alcohol,breaks=c(0,5,10,15,max(residual_sugar_1$alcohol)))  
str(train)
CrossTable(fx_alcohol2 , residual_sugar_1$quality, type = "r")
```
...
Estamos utilizando a variável "density" porque ela tem uma alta correlação com a qualidade do vinho onde define qual é o "Corpo do vinho".
