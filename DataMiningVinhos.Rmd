...
axis(1, labels = FALSE)
text(x =  seq_along(colnames(outliersBase)), y = par("usr")[3] - 1, srt = 45, adj = 1,labels = colnames(teste), xpd = TRUE)
...
baseVinhoBrancoAtual = select(baseVinhoBranco, freesulfurdioxide,residualsugar,alcohol,pH,
              density,sulphates,fixedacidity,chlorides,volatileacidity,Vinho,
              quality)
...
#treinando o modelo
linear <- lm(quality ~
               freesulfurdioxide+residualsugar+alcohol+pH+density+sulphates+fixedacidity+volatileacidity+qualidadeVinho, data=train)
...
#library(DAAG)
cv.lm(data=baseVinhoBrancoAtual, linear, m=2) # 3 fold cross-validation

```

Precisamos criar a arvore de regressão
```{r}
...
```{r}
ajuste_glm <- glm(qualidadeVinho ~ density, data = train, family = binomial)
summary(ajuste_glm)
...
reg_logistica_probit <- glm(qualidadeVinho ~ density, data = train, family = binomial(link = "probit"))
...
ggplot(train, aes(x=qualidadeVinho, y=alcohol)) + 
  geom_point() + 
...
ggplot(train, aes(x=qualidadeVinho, y=density)) + 
  geom_point() + 
  stat_smooth(aes(colour = "Logit"), method="glm", family=binomial, se=FALSE) +
  stat_smooth(aes(colour = "Probit"), method="glm", family=binomial(link = "probit"), se=FALSE) +
  stat_smooth(aes(colour = "Complementar Log-Log"), method="glm", family=binomial(link = "cloglog"), se=FALSE) +
...
modelo_acuracia <- lm(quality ~ fixedacidity+volatileacidity+residualsugar+chlorides+freesulfurdioxide+density+pH+sulphates+alcohol)
